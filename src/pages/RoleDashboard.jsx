...
export default function RoleDashboard({role}){const {logout,profile,school,demo,setProfile}=useAuth(),{lang, setLang, t}=useLanguage(),navigate=useNavigate(),settings=cfg[role],first=settings.groups[0][1][0],[selected,setSelected]=useState(first),[mobileOpen,setMobileOpen]=useState(false),[liveStats,setLiveStats]=useState(settings.stats),table=settings.tables[selected],canCreate=['school_admin','teacher','super_admin'].includes(role),[studentId,setStudentId]=useState(null),[notifOpen, setNotifOpen]=useState(false),[sidebarCollapsed, setSidebarCollapsed]=useState(false);
  ...
  const icons = {
    ...
    'My submissions': '📤', 'My results': '📊', 'Fees & payments': '💳'
  };

  const mainNavItems = settings.groups[0][1].slice(0, 4); // Pick top 4 for mobile bottom nav

useEffect(()=>{if(demo||!profile?.school_id||role!=='school_admin')return;const load=async()=>{const tables=['students','profiles','classes','subjects'];const counts=await Promise.all(tables.map(table=>supabase.from(table).select('*',{count:'exact',head:true}).eq('school_id',profile.school_id)));setLiveStats(current=>current.map(([label,value],index)=>index<4?[label,counts[index].count??value]:[label,value]))};load()},[demo,profile?.school_id,role]);

useEffect(() => {
  if (school?.primary_color) {
    document.documentElement.style.setProperty('--green', school.primary_color);
  }
}, [school]);
...
useEffect(() => {
    if (!profile) return;
    if (demo || profile.school_id === 'demo-school-id') {
      if (role === 'student' || role === 'parent') setStudentId('ST-DEMO-1');
      return;
    }
    if (role !== 'student') return;
    const load = async () => {
      const { data } = await supabase.from('students').select('id').eq('profile_id', profile.id).maybeSingle();
      if (data) setStudentId(data.id);
    };
    load();
  }, [profile, role, demo]);const choose=item=>{if(item==='Messages')return navigate('/chat');if(item==='Events')return navigate('/calendar');if(item==='Timetable')return navigate('/timetable');if(item==='Gradebook')return navigate('/gradebook');if(item==='Meetings')return navigate('/meetings');if(item==='Broadcast')return navigate('/broadcast');if(item==='Lesson Plans')return navigate('/lessons');if(item==='My Profile')return navigate('/profile');if(item==='Library')return navigate('/library');if(item==='Course Builder')return navigate('/course-builder');if(item==='My Courses')return navigate('/course-builder');if(item==='Courses')return navigate('/course-builder');if(item==='Quizzes')return navigate('/quizzes');if(item==='Audit Trail')return navigate('/audit-trail');if(item==='Settings')return navigate('/settings');if(item==='Behavior Log')return navigate('/behavior-log');setSelected(item);setMobileOpen(false)};
    ...
    <div className="side-header">
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <Link className="brand light" to="/">
          {school?.logo_url ? (
            <img src={school.logo_url} alt="Logo" style={{height: '32px', marginRight: '10px'}} />
          ) : null}
          <span>{school?.name || 'SmartSchool'}</span>
        </Link>
        <button className="side-close-btn" onClick={() => setMobileOpen(false)}>×</button>
      </div>
      <p className="portal-label">{role.replace('_',' ').toUpperCase()}</p>
    </div>
  ...
  <main className="portal-main">
    <header className="main-header">
      <div className="header-left">
        <button className="menu-toggle-btn" onClick={() => setMobileOpen(true)}>☰</button>
        <button className="desktop-hamburger" onClick={() => setSidebarCollapsed(!sidebarCollapsed)}>☰</button>
        <div className="title-stack">
          <p className="eyebrow">{school?.name || 'SmartSchool'}</p>
          <h1>{t(selected.toLowerCase().replaceAll(' ', '_'))}</h1>
        </div>
      </div>
      <div className="user-profile">
        <button className="notif-bell" onClick={() => setNotifOpen(true)}>🔔</button>
        <select className="lang-select" value={lang} onChange={(e) => setLang(e.target.value)}>
          <option value="en">EN</option>
          <option value="sw">SW</option>
        </select>
        <div className="user-info">
          <b>{profile?.full_name||'SmartSchool user'}</b>
    ...
    <div className="content-area">
      {selected === 'Analytics' ? (
        ...
        <>
          {role === 'parent' && (
            <div className="child-switch glass-card">
              <b>{t('viewing_child')}:</b>
              <select defaultValue="Amina Hassan" onChange={e => setStudentId(e.target.value)}>
                <option value="ST-DEMO-1">Amina Hassan - Grade 5A</option>
                <option value="ST-DEMO-2">Yusuf Hassan - Grade 8B</option>
                <option value="ST-DEMO-3">Mary Hassan - Grade 10A</option>
              </select>
            </div>
          )}
          ...
          <section className="stats-grid">
            {liveStats.map(([label, value]) => (
              <article key={label} className="stat-card glass-card">
                <p>{t(label.toLowerCase().replace(' ', '_'))}</p>
                <strong>{value}</strong>
              </article>
            ))}
          </section>
          ...
          {(role === 'student' || role === 'parent') && studentId && (
            <>
              <div className="quick-actions-bar page-transition">
                <Link to={`/report-card/${studentId}`} className="button blue-button">
                  View Academic Report Card 📊
                </Link>
              </div>
              <BadgeSystem studentId={studentId} />
            </>
          )}
        ...
        /* Module View (Non-Dashboard) */
        <div className="module-view-container">
           {role === 'parent' && table !== 'students' && (
            <div className="child-switch glass-card mini-switch">
              <b>{t('viewing_child')}:</b>
              <select defaultValue="Amina Hassan" onChange={e => setStudentId(e.target.value)}>
                <option value="ST-DEMO-1">Amina Hassan</option>
                <option value="ST-DEMO-2">Yusuf Hassan</option>
                <option value="ST-DEMO-3">Mary Hassan</option>
              </select>
            </div>
          )}
          <ModulePanel
...
function DashboardHighlights({role,profile, t}){const [items,setItems]=useState([]),[pulse,setPulse]=useState(role==='parent'?'92%':'94%');useEffect(()=>{const demoItems=role==='teacher'?['3 assignments awaiting review','Grade 7A attendance is ready','Next class starts at 10:30']:role==='student'?['2 assignments due this week','New result posted','Fee statement is available']:role==='parent'?['Amina improved in Mathematics','Parent meeting is Friday','One payment is due soon']:['School announcement published','12 assignments need attention','Attendance report is ready'];if(!supabase||!profile?.school_id){setItems(demoItems);return}const load=async()=>{const [announcements,assignments,results,attendance]=await Promise.all([supabase.from('announcements').select('title').eq('school_id',profile.school_id).order('created_at',{ascending:false}).limit(1),supabase.from('assignments').select('title').eq('school_id',profile.school_id).order('due_date').limit(1),supabase.from('results').select('id').eq('school_id',profile.school_id).limit(1),supabase.from('attendance').select('status').eq('school_id',profile.school_id).eq('attendance_date',new Date().toISOString().slice(0,10))]);const live=[announcements.data?.[0]?.title,assignments.data?.[0]?.title,results.data?.length?'A new result is available':null].filter(Boolean);setItems(live.length?live:demoItems);const present=attendance.data?.filter(row=>row.status==='present').length||0;setPulse(attendance.data?.length?`${Math.round((present/attendance.data.length)*100)}%`:'--')};load()},[role,profile?.school_id]);return <section className="dashboard-highlights"><div className="panel highlight-card glass-card"><p className="eyebrow blue-eyebrow">TODAY</p><h2>{t('next_actions')}</h2>{items.map((item,index)=><div className="highlight-row" key={item}><i>0{index+1}</i><span>{item}</span><b>→</b></div>)}</div><div className="panel pulse-card glass-card"><p className="eyebrow blue-eyebrow">SCHOOL PULSE</p><h2>{t('attendance')}</h2><div className="pulse-ring">{pulse}</div><p>{pulse==='--'?'No attendance recorded yet':'Present today'}</p></div></section>}
